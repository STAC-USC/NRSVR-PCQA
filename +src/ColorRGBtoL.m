function ColorLab = ColorRGBtoL(ColorRGB)
	...
	ColorLab = zeros(size(ColorRGB,1),1);

    IndexL1 = find(Y > 0.008856);
    IndexL2 = find(Y <= 0.008856);
    ColorLab(IndexL1,1) = 116.0 * Y(IndexL1).^(1/3) - 16.0;
    ColorLab(IndexL2,1) = 116.0 * ((841.0 / 108.0) * Y(IndexL2) + 4.0 / 29.0) - 16.0;
